...
async def run_all(mode_name: str, sequential: bool, output_file: str) -> None:
    ...
    with open(f"tests/results/{output_file}", "w") as f:
    ...
    print(f"{'='*60}")
    print(f"Results saved to tests/results/{output_file}")
...
async def main() -> None:
    ...
    with open("tests/results/enriched_sequential_results.json") as f:
        seq = json.load(f)
    with open("tests/results/enriched_oneshot_results.json") as f:
        one = json.load(f)
    ...
    print(f"\nComparison saved to tests/results/enriched_comparison.md")
